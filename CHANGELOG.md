...
### Added
- Content-API: Exportvertrag `flowcore.content-api.page 2.0` (Reaudit FC-RA-20260911). Jede Seitenantwort trägt jetzt Eltern-ID und Sortierung, fachliche Metadaten (Marke, Markenebene, Herkunftsfassung, Prüfzyklus, führendes System), `governance` mit Wert, Herkunft und Bedeutung je Statusangabe, `contentLinks` mit Feld und Tabellenposition jedes Seitenverweises sowie Unterseiten mit `id` und `sortOrder` in gespeicherter Reihenfolge. Alle Links verlassen die API absolut gegen `linkBase`. Dokumentiert in `docs/30-CONTENT-API.md` und in der OpenAPI-Beschreibung (1.1.0), belegt in `content-api-exportvertrag.test.ts`
- Content-API: `?format=markdown|text|full` für `GET /v1/pages/{id}`; Standard `markdown` liefert den Inhalt genau einmal und ohne Editor-JSON (1.600 KB -> 517 KB über die 41 Markensystem-Seiten, mit gzip 192 KB)

### Changed
- Content-API, Exportvertrag 2.1 (Rückfrage eines angebundenen Systems vom 12.09.2026): Nicht gepflegte Statusfelder liefern `wert: null` mit `herkunft: "nicht gepflegt …"` statt eines Standardwerts. Zuvor trugen `decisionStatus` den Wert `proposed` und `sourcePriority` den Wert `1` auf **allen** Seiten — ein Zielsystem, das das naiv liest, schließt daraus, im gesamten Qualitätsmanagementsystem sei keine Seite beschlossen. Ein leeres Feld wird ignoriert, ein falsch gefülltes ausgewertet. Zusätzlich stehen die Statusangaben nur noch in `governance` und nicht mehr zusätzlich als flache Felder in der Antwort; Copilot-Connector und Graph lesen die Projektion unverändert

### Fixed
- Content-API: Seitenlinks in Tabellen verloren ihre Ziel-UUID. `serializeTable` verarbeitete Zellen nur über den Klartextpfad, während Absätze denselben Inhalt über den Markdown-Pfad rendern — ein `wikiLink` trägt sein Ziel aber nur dort. Betroffen waren 171 Vorkommen in 20 Markenseiten; in 15 Dateien verschwanden dadurch 58 Zielbeziehungen vollständig aus dem Abzug. Zellen laufen jetzt durch denselben Inline-Renderer; Pipes werden maskiert, harte Umbrüche werden `<br>`, Spaltenzahl und Zelltexte bleiben unverändert (`tabellenlinks-serializer.test.ts`, am PROD-Bestand mit 171/171 nachgemessen)
- Content-API: Unterseiten kamen in der Abrufreihenfolge der Datenbank statt nach `sort_order` — vier Register waren dadurch falsch sortiert
- Doku und Einstellungsdialog sagten zu, die Schnittstellenbeschreibung sei ohne Schlüssel abrufbar — tatsächlich greift die globale Anmeldesperre auch dort. Beides sagt jetzt, was gilt: Die Beschreibung enthält keine Inhalte, verlangt aber denselben Header wie die Daten; für Werkzeuge ohne Kopfzeilen steht der Weg über eine vorab geladene Datei in `docs/30-CONTENT-API.md`
- Markensystem: operative Verweise binden nicht mehr sprachlich an die Herkunftsfassung 1.0, sondern an die veröffentlichte FlowCore-Revision (18 Seiten, 35 Fundstellen, Auftrag C-01); historische Angaben, Herkunftsarchiv und Änderungsverlauf 1.0 bleiben unverändert
- Waechter ueber die ZUORDNUNG von Rumpfschema und Route (`routen-schema-zuordnung.test.ts`). Bezug ist die `operationId` der Spezifikation, nicht der Schemaname: Orval benennt das erzeugte Zod-Rumpfschema deterministisch danach (`reviewDeletionRequest` -> `ReviewDeletionRequestBody`). Der Spec-Schemaname taugt nicht als Bezug, weil dort geteilte Komponenten stehen duerfen (`PATCH /tags/{id}` verweist zu Recht auf `CreateTag`). Geprueft wird nur, was aus `@workspace/api-zod` kommt — selbst definierte Schemata einer Routendatei sind bewusste Handarbeit. Mit Gegenprobe: der kuenstlich wiederhergestellte Fehler vom 10.09.2026 laesst den Waechter anschlagen und wird woertlich benannt

### Fixed
- Arbeitskopien: drei Routen prueften gegen das Schema einer fremden Operation, gefunden vom neuen Waechter. `submit` prueft jetzt `SubmitWorkingCopyBody` statt des Revisions-Schemas — jenes kennt `changeType` und `changeSummary` gar nicht, weshalb **`changeType` bisher vollstaendig ungeprueft durchlief**; ein unsinniger Wert waere angekommen. `return-for-changes` und `approve` ebenso auf ihre eigenen Schemata gezogen. Beim Freigeben war zusaetzlich `nextReviewDate: zod.date()` gebunden — ein Date-Objekt, das JSON nicht transportieren kann: Heute sendet die Oberflaeche das Feld nicht, wer es ergaenzt haette, waere auf 400 gelaufen
- Loeschanfragen lassen sich wieder genehmigen und ablehnen (gemeldet 10.09.2026). An `POST /deletion-requests/:id/review` hing `CreateDeletionRequestBody` — das Schema fuers ANLEGEN, mit den Pflichtfeldern `nodeId` und `reason`. Die Oberflaeche sendet `{ decision, comment? }`, wie es die Spezifikation als `ReviewDeletionRequestBody` beschreibt. Die Rumpfpruefung war damit unerfuellbar: jede Entscheidung endete mit 400 "Validierungsfehler", fuer jeden Benutzer. Das passende Schema war die ganze Zeit erzeugt vorhanden und wurde nur nicht eingesetzt; der Import des Anlege-Schemas entfaellt, weil die Anlege-Route ihren Rumpf gar nicht prueft. Dritter Fall derselben Familie nach `GET /nodes/:id/revisions` und `POST /media/upload`. Neu abgesichert mit 9 Tests: `review-durchlauf.test.ts` ruft die Route ueber die echte Middleware-Kette auf, `review-rumpfschema.test.ts` wacht ueber die Zuordnung von Schema und Route
- Anmeldung fragt wieder, mit welchem Konto (gemeldet 10.09.2026). `getAuthCodeUrl` wurde ohne `prompt` aufgerufen; Entra nimmt dann stillschweigend das im Browser bereits angemeldete Konto. Wer zwei Konten hat — Arbeits- und Administratorkonto — landete mit dem falschen hier, bekam `?auth_error=group_not_authorized` und hatte keine Moeglichkeit, das andere zu waehlen. Jetzt `prompt: "select_account"`; abgesichert in `kontoauswahl.test.ts`
