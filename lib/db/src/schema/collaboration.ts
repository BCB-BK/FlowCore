...
export const pageWatchersTable = pgTable(
  ...
  (table) => [
    index("idx_page_watchers_principal").on(table.principalId),
    uniqueIndex("idx_page_watchers_unique").on(table.nodeId, table.principalId),
  ],
);

export const pageCommentsTable = pgTable(
  "page_comments",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    nodeId: uuid("node_id")
  ...
  (table) => [
    index("idx_page_comments_author").on(table.authorId),
    index("idx_page_comments_node").on(table.nodeId),
    index("idx_page_comments_parent_comment").on(table.parentCommentId),
    index("idx_page_comments_revision").on(table.revisionId),
  ],
);

export const pageVerificationsTable = pgTable(
  "page_verifications",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    nodeId: uuid("node_id")
  ...
  (table) => [
    index("idx_page_verifications_node").on(table.nodeId),
    index("idx_page_verifications_verified_by").on(table.verifiedBy),
  ],
);
...
export const favoritesTable = pgTable(
  ...
  (table) => [
    index("idx_favorites_node").on(table.nodeId),
...
export const notificationsTable = pgTable(
  ...
  (table) => [
    ...
    index("idx_notifications_node").on(table.nodeId),
