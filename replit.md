...
- **Core Features:**
    ...
    - **Working Copies (Arbeitskopie):** Full lifecycle management — create, update (autosave), submit, return-for-changes, approve, publish, cancel, unlock. One active WC per node (advisory lock). Publish creates a new content_revision and updates publishedRevisionId. All endpoints have node-scoped RBAC authorization. Event audit trail tracked in `working_copy_events`.
