...
const SIPOC_COLUMNS = [
  {
    ...
    label: "Suppliers",
    color: "text-blue-600",
    description: "Wer liefert die Eingaben?",
  },
  {
    key: "inputs",
    label: "Inputs",
    color: "text-green-600",
    description: "Welche Eingaben werden benötigt?",
  },
  {
    key: "process",
    label: "Process",
    color: "text-primary",
    description: "Was sind die Hauptschritte?",
  },
  {
    key: "outputs",
    label: "Outputs",
    color: "text-amber-600",
    description: "Was wird produziert?",
  },
  {
    key: "customers",
    label: "Customers",
    color: "text-purple-600",
    description: "Wer erhält die Ergebnisse?",
  },
] as const;
...
export function SipocEditor({
  ...
  const hasContent =
    value && Object.values(value).some((v) => v && String(v).trim());
          ...
          <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
            {SIPOC_COLUMNS.map((col) => (
              <div key={col.key} className="space-y-1.5">
                <label className={`text-xs font-medium ${col.color}`}>
                  {col.label}
                </label>
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setDraft({ ...draft, [col.key]: e.target.value })
                  }
