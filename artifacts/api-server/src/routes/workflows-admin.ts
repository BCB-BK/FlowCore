...
workflowsAdminRouter.post("/admin/workflows", ...auth, async (req, res) => {
  try {
    const { name, description, isDefault, isActive, enforceSoD, steps } =
      req.body;
    if (!name) {
...
workflowsAdminRouter.delete(
  ...
  async (req, res) => {
    try {
      ...
      const message = err instanceof Error ? err.message : "Unknown error";
      const status =
        message === "Der Standard-Workflow kann nicht gelöscht werden."
          ? 400
          : 500;
      res.status(status).json({
        error: status === 400 ? message : sanitizeInternalError(message),
      });
    }
  },
);

workflowsAdminRouter.get(
...
workflowsAdminRouter.put(
  "/admin/workflow-assignments/:pageType",
  ...auth,
  async (req, res) => {
    try {
...
workflowsAdminRouter.post(
  "/admin/notification-rules/seed",
  ...auth,
  async (_req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      if (!deleted) {
        res
          .status(404)
          .json({ error: "Benachrichtigungsregel nicht gefunden" });
        return;
      }
      res.json({ success: true });
    } catch (err) {
...
workflowsAdminRouter.post(
  "/admin/flowcore-account/test",
  ...auth,
  async (req, res) => {
    try {
      const { upn } = req.body;
      const targetUpn =
        upn ?? (await getSystemSetting("flowcore_account_upn")) ?? "";
      ...
      try {
        ...
        res.json({
          success: false,
          message:
            "App-Token konnte nicht abgerufen werden. Prüfen Sie die Entra-Konfiguration.",
        });
        return;
      }
      ...
      if (!accessToken) {
        res.json({
          success: false,
          message:
            "Kein Zugriffstoken verfügbar. Prüfen Sie die Entra-Konfiguration.",
        });
        return;
      }
      ...
      if (!graphRes.ok) {
        ...
        logger.debug(
          { status: graphRes.status, errText, targetUpn },
          "FlowCore account test failed",
        );
        res.json({
          success: false,
