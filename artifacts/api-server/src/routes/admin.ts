...
const PII_FIELD_PATTERNS =
  /^(actor|author|user|submitted|approved|returned|created|updated|reviewed|assigned|delegat|unlock|cancel|comment|ip|email|name)/i;
  ...
  async (req, res) => {
    try {
      ...
      if (req.query.resourceType)
        opts.resourceType = req.query.resourceType as string;
      if (req.query.resourceId)
        opts.resourceId = req.query.resourceId as string;
  ...
  async (req, res) => {
    try {
      ...
      if (format === "csv") {
        ...
        const header =
          "id;eventType;action;actorId;resourceType;resourceId;correlationId;createdAt\n";
  ...
  async (req, res) => {
    try {
      ...
      if (!ALLOWED_SYSTEM_SETTING_KEYS.has(key)) {
        res
          .status(400)
          .json({ error: `Unbekannter Einstellungs-Schlüssel: ${key}` });
        return;
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      logger.info(
        {
          sid,
          terminatedBy: req.user?.principalId,
          targetUser: sessionUser?.principalId,
        },
