...
router.get(
  ...
  async (_req, res) => {
    ...
    const result = CONFIDENTIALITY_LEVELS.filter((l) => l !== "public").map(
      (level) => ({
        level,
        assignments: assignments
          ...
          .map((a) => ({
            ...
            displayName:
              principalMap.get(a.principalId)?.displayName ?? "Unbekannt",
            email: principalMap.get(a.principalId)?.email ?? null,
            principalType:
              principalMap.get(a.principalId)?.principalType ?? "user",
...
router.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      res.status(400).json({
        error: `Invalid level: ${level}. Must be internal, confidential, or strictly_confidential.`,
      });
      return;
    }
