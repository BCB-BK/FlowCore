...
export async function ensureDevPrincipals() {
  const { getDevUsers } = await import("./auth.service");
  for (const user of getDevUsers()) {
    ...
    await db
      .update(principalsTable)
      .set({ id: user.principalId })
      .where(eq(principalsTable.id, principalId));

    for (const role of user.roles) {
      const actualPrincipalId = user.principalId;
      const existingRoles = await db
        .select({ id: roleAssignmentsTable.id })
        .from(roleAssignmentsTable)
        .where(
          and(
            eq(roleAssignmentsTable.principalId, actualPrincipalId),
            eq(roleAssignmentsTable.role, role as "system_admin"),
            eq(roleAssignmentsTable.isActive, true),
          ),
        );
      if (existingRoles.length === 0) {
        await db.insert(roleAssignmentsTable).values({
          principalId: actualPrincipalId,
  ...
  logger.info("Dev principals seeded");
}
